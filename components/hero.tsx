...
export default function Hero() {
  return (
    <section className="relative min-h-screen dot-grid overflow-hidden">
      ...
      >
        {/* Mock dashboard — real screenshot would go here */}
        <div className="w-full h-full bg-[#0d0d0d] flex flex-col">
          <div className="h-10 bg-[#111] border-b border-white/5 flex items-center gap-3 px-4">
            <div className="flex gap-1.5">
              {[0,1,2].map(i => <div key={i} className="h-3 w-3 rounded-full bg-white/10" />)}
            </div>
            <div className="flex-1 h-5 rounded bg-white/5 text-[11px] text-white/20 flex items-center px-3 font-mono">
          ...
          <div className="flex-1 p-4 grid grid-cols-4 gap-3">
            {[
              ...
              <div key={s.label} className="rounded-xl bg-white/[0.03] border border-white/5 p-3 flex flex-col justify-between">
                <p className="text-[10px] text-white/30 font-mono">{s.label}</p>
                <div>
                  <p className="text-xl font-black text-white">{s.val}</p>
                  <p className="text-[10px] text-green-400 font-mono mt-0.5">{s.change}</p>
                </div>
              </div>
            ))}
            <div className="col-span-4 rounded-xl bg-white/[0.03] border border-white/5 p-4">
              <p className="text-[10px] text-white/30 font-mono mb-2">90-day follower growth</p>
              ...
              {["YouTube", "TikTok", "Instagram", "Facebook"].map(p => (
                <div key={p} className="flex items-center justify-between py-0.5">
                  ...
                  <div className="h-1.5 w-16 rounded-full bg-white/5 overflow-hidden">
                    <div className="h-full bg-blue-500/50 rounded-full" style={{ width: `${Math.random() * 40 + 40}%` }} />
                  </div>
                </div>
              ))}
            </div>
            <div className="col-span-2 rounded-xl bg-white/[0.03] border border-white/5 p-3">
              ...
              <p className="text-xs text-white/50 leading-relaxed">Creator · Lifestyle & Tech · Growing audience aged 18–34 across 4 platforms.</p>
              <div className="mt-2 flex gap-1 flex-wrap">
                {["Educational", "Behind-scenes", "Trending"].map(t => (
                  <span key={t} className="rounded border border-blue-500/20 bg-blue-500/5 px-1.5 py-0.5 text-[10px] text-blue-400/70 font-mono">{t}</span>
                ))}
              </div>
            </div>
          </div>
